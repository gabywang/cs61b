...
class program:
    ...
    def check_move(self, who, move):
        ...
        patn = WHITE_MOVE if who == "white" else BLACK_MOVE
    ...
    def send_recv_moves(self, send_first, first_mover, end_patn):
        ...
        second_mover = "black" if first_mover == "white" else "white"
        if send_first:
    ...
    def check_output(self):
        ...
        if search('Exception', line):
            self.error("uncaught exception occurwhite: {}", line.rstrip())
    ...
    COMMAND_PATTERNS = (
        ...
        (r"@(white)\.\.\.\s*(.*)", play_self),
        (r"@(black)\.\.\.\s*(.*)", play_self),
        (r"@send (white|black)\.\.\.\s*(.*)", send_moves),
        (r"@recv (white|black)\.\.\.\s*(.*)", receive_moves),
        (r"@", bad_command)
        )
