...
def correctProgramOutput(expected, actual, last_groups, is_regexp):
    ...
    def printerror(expected, actual, last_groups, is_regexp):
        print("\nEXPECTED: \n" + expected + "\n")
        print("ACTURAL: \n" + actual + "\n")
    ...
    if is_regexp:
        try:
            if not Match(expected.rstrip() + r"\Z", actual) \
                ...
                printerror(expected, actual, last_groups, is_regexp)
                return False
        except:
        ...
        while True:
            ...
            if Match(r'\s*#', line) or Match(r'\s+$', line):
                ...
                if not correctFileOutput(Group(1), Group(2), cdir):
                    ...
                    print(contents(join(cdir, Group(1))))
                    print(contents(join(src_dir, Group(2))))
